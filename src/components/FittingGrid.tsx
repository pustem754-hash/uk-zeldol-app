...
export function FittingGrid({ items, personImageUrl, userId = 1 }: FittingGridProps) {
  ...
  const handleTryOn = useCallback(
    async (item: FittingItem, index: number) => {
      setError(null);
      setResultImage(null);
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {items.map((item, i) => (
          ...
          >
            ...
            <div className="w-full mt-2 pb-6 md:pb-2">
              <button
                onClick={() => handleTryOn(item, i)}
                disabled={loadingIndex !== null}
                className="w-full bg-marquis-primary hover:bg-marquis-primary/80 text-white text-xs py-2.5 px-3 rounded-lg transition-colors disabled:opacity-50"
              >
                {loadingIndex === i
                  ? 'Обработка...'
                  : `Примерить — ${PRICING_DISPLAY.VIRTUAL_TRY_ON}`}
              </button>
            </div>
          </div>
        ))}
